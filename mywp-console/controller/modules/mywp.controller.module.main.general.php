...
if ( ! class_exists( 'MywpControllerModuleConsoleMainGeneral' ) ) :

final class MywpControllerModuleConsoleMainGeneral extends MywpControllerAbstractModule {

  static protected $id = 'console_main_general';
  ...
  private static function is_current_plugin( $plugin_file_name = false ) {
    ...
    if ( strpos( MYWP_CONSOLE_PLUGIN_BASENAME , $plugin_file_name ) === false ) {
  ...
  public static function load_plugins() {
    ...
    if( is_multisite() ) {
      ...
      add_filter( 'network_admin_plugin_action_links_' . MYWP_CONSOLE_PLUGIN_BASENAME , array( __CLASS__ , 'plugin_action_links' ) , 10 , 4 );

    } else {

      add_filter( 'plugin_action_links_' . MYWP_CONSOLE_PLUGIN_BASENAME , array( __CLASS__ , 'plugin_action_links' ) , 10 , 4 );

    }

    add_action( 'in_plugin_update_message-' . MYWP_CONSOLE_PLUGIN_BASENAME , array( __CLASS__ , 'in_plugin_update_message' ) , 10 , 2 );
  ...
  public static function plugin_row_meta( $plugin_meta , $plugin_file , $plugin_data , $status ) {
    ...
    $plugin_meta = apply_filters( 'mywp_console_plugin_row_meta' , $plugin_meta , $plugin_file , $plugin_data , $status );
  ...
  public static function plugin_action_links( $actions , $plugin_file , $plugin_data , $context ) {
    ...
    $actions = apply_filters( 'mywp_console_plugin_action_links' , $actions , $plugin_file , $plugin_data , $context );
  ...
  public static function in_plugin_update_message( $plugin_data , $response ) {
    ...
    printf( __( 'There is a new version of %1$s available. <a href="%2$s" %3$s>View version %4$s details</a>.' ) , $response->new_version , $plugin_info['github'] , '' ,  MYWP_CONSOLE_NAME );
  ...
  public static function admin_print_footer_scripts() {
    ...
    printf( 'tr#%s-update .update-message p { display: none; }' , MYWP_CONSOLE_PLUGIN_DIRNAME );
    printf( 'tr#%s-update .update-message p.show { display: block; }' , MYWP_CONSOLE_PLUGIN_DIRNAME );
...
MywpControllerModuleConsoleMainGeneral::init();
