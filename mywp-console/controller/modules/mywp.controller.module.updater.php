...
if ( ! class_exists( 'MywpControllerModuleConsoleUpdater' ) ) :

final class MywpControllerModuleConsoleUpdater extends MywpControllerAbstractModule {

  static protected $id = 'console_updater';

  static private $schedule_hook = 'mywp_console_version_check';
  ...
  public static function site_transient_update_plugins( $site_transient ) {
    ...
    $plugin_info = MywpConsoleApi::plugin_info();

    $update_plugin = array(
      'id' => MYWP_CONSOLE_PLUGIN_BASENAME,
      'slug' => MYWP_CONSOLE_PLUGIN_DIRNAME,
      'plugin' => MYWP_CONSOLE_PLUGIN_BASENAME,
      'new_version' => $latest,
    ...
    $site_transient->response[ MYWP_CONSOLE_PLUGIN_BASENAME ] = (object) $update_plugin;
  ...
  public static function get_remote() {
    ...
    $transient_key = 'mywp_console_updater_remote';
    ...
    if( empty( $remote_result ) ) {
      ...
      $error->add( 'not_results' , __( 'Connection lost or the server is busy. Please try again later.' , 'mywp-console' ) );
    ...
    if( $remote_code !== 200 ) {
      ...
      if( ! empty( $remote_body ) ) {
        ...
        if( ! empty( $maybe_json ) && ! empty( $maybe_json->message ) ) {
          ...
          $error->add( 'invalid_json' , sprintf( '[%d] %s' , $remote_code , __( 'An error has occurred. Please reload the page and try again.' , 'mywp-console' ) ) );

        }

      } else {

        $error->add( 'invalid_connection' , sprintf( '[%d] %s' , $remote_code , __( 'Connection lost or the server is busy. Please try again later.' , 'mywp-console' ) ) );
    ...
    if( empty( $remote_body ) ) {
      ...
      $error->add( 'invalid_remote_body' , __( 'An error has occurred. Please reload the page and try again.' , 'mywp-console' ) );
  ...
  public static function get_latest() {
    ...
    $transient_key = 'mywp_console_updater';
    ...
    if( ! is_array( $maybe_remote_json ) or empty( $maybe_remote_json[0] ) ) {
      ...
      $error->add( 'invalid_remote_json' , __( 'Invalid remote Json data. Please try again in an hour.' , 'mywp-console' ) );
    ...
    if( ! is_object( $remote_json ) or ! isset( $remote_json->name ) or ! isset( $remote_json->zipball_url ) or ! isset( $remote_json->tarball_url ) ) {
      ...
      $error->add( 'invalid_json' , __( 'Invalid results. Sorry maybe update format changed.' , 'mywp-console' ) );
  ...
  public static function is_latest() {
    ...
    $latest_compare = version_compare( $latest , MYWP_CONSOLE_VERSION , '<=' );
...
MywpControllerModuleConsoleUpdater::init();
